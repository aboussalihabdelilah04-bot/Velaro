...
(function() {
    ...
    var reservation = reservations.find(function(r) { return r.id === reservationId || r._id === reservationId || r.reference === reservationId; });

    if (!reservation && reservationId) {
        fetch('/api/public/bookings/' + encodeURIComponent(reservationId))
            .then(function(res) {
                if (!res.ok) throw new Error('Not found');
                return res.json();
            })
            .then(function(serverBooking) {
                renderConfirmation(serverBooking);
            })
            .catch(function() {
                showNotFound();
            });
    ...
    var pricePerDay = reservation.pricePerDay || reservation.totalPrice || 0;
    if (!pricePerDay && days > 0) pricePerDay = Math.round(reservation.totalPrice / days);
    var grandTotal = reservation.totalPrice || (pricePerDay * days);
    var clientFirstName = reservation.firstName || (reservation.clientName || '').split(' ')[0] || '';
    var clientLastName = reservation.lastName || (reservation.clientName || '').split(' ').slice(1).join(' ') || '';
    var clientEmail = reservation.email || reservation.clientEmail || '';
    var clientPhone = reservation.phone || reservation.clientPhone || '';
    var resId = reservation.id || reservation._id || reservation.reference || '';

    /* --- Type labels --- */
    var typeLabels = {
        voiture: 'Location de Voiture', car: 'Location de Voiture',
        moto: 'Location de Moto', motorcycle: 'Location de Moto',
        maison: 'Location d\'H\u00e9bergement', villa: 'Location d\'H\u00e9bergement',
    ...
    var statusLabels = { pending: 'En attente', confirmed: 'Confirm\u00e9e', completed: 'Termin\u00e9e', cancelled: 'Annul\u00e9e' };
            ...
            '<div style="background:var(--white);border:1px solid var(--gray-200);border-radius:16px;overflow:hidden;">' +
                '<div style="background:var(--primary);color:white;padding:1.5rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">' +
                    '<div>' +
                        ...
                        '<div style="font-size:1.25rem;font-weight:700;">#' + resId + '</div>' +
                    '</div>' +
                ...
                '<div style="padding:1.5rem;">' +
                    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">' +
                        '<div>' +
                            ...
                            '<p style="font-weight:600;">' + clientFirstName + ' ' + clientLastName + '</p>' +
                            '<p style="font-size:0.9rem;color:var(--gray-600);">' + clientEmail + '</p>' +
                            '<p style="font-size:0.9rem;color:var(--gray-600);">' + clientPhone + '</p>' +
                        '</div>' +
                        '<div>' +
                            '<h4 style="font-size:0.8rem;color:var(--gray-500);text-transform:uppercase;margin-bottom:0.5rem;">Produit</h4>' +
                            '<p style="font-weight:600;">' + (typeLabels[reservation.productType || reservation.type] || reservation.productType || reservation.type) + '</p>' +
                    ...
                    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">' +
                        ...
                        '<div>' +
                            ...
                            '<p style="font-size:0.9rem;"><strong>Prix unitaire :</strong> ' + formatPrice(pricePerDay) + '</p>' +
    ...
    if (qrEl) {
        var qrData = JSON.stringify({
            reservation: reservation.id || reservation._id || reservation.reference,
            client: (reservation.clientName || (reservation.firstName + ' ' + reservation.lastName)),
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', function() {
            var resId = reservation.id || reservation._id || reservation.reference;
            var clientName = reservation.clientName || (reservation.firstName + ' ' + reservation.lastName);
            var printWindow = window.open('', '_blank');
            printWindow.document.write(
                '<!DOCTYPE html><html><head><title>Re\u00e7u VelaroCar - ' + resId + '</title>' +
                '<style>' +
                ...
                '<div class="header">' +
                    ...
                    '<div class="info"><div><strong>R\u00e9c\u00e9piss\u00e9 de r\u00e9servation</strong></div><div>N\u00b0 ' + resId + '</div><div>Date : ' + new Date().toLocaleDateString('fr-FR') + ' \u00e0 ' + new Date().toLocaleTimeString('fr-FR') + '</div></div>' +
                '</div>' +
                '<div class="section"><h3>Informations Client</h3>' +
                    '<div class="row"><strong>Nom complet :</strong> ' + clientName + '</div>' +
                    '<div class="row"><strong>Email :</strong> ' + (reservation.clientEmail || reservation.email || '') + '</div>' +
                    '<div class="row"><strong>T\u00e9l\u00e9phone :</strong> ' + (reservation.clientPhone || reservation.phone || '') + '</div>' +
                '</div>' +
                '<div class="section"><h3>D\u00e9tails de la R\u00e9servation</h3>' +
                    '<div class="row"><strong>Type :</strong> ' + (typeLabels[reservation.productType || reservation.type] || reservation.productType || reservation.type) + '</div>' +
